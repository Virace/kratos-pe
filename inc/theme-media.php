...
add_action('admin_enqueue_scripts', 'init_emwi');
        ...
        <div class="row2">
            <?php if ('grid' === $media_library_mode) :  // FIXME: seems that media_library_mode being empty also means grid mode ?>
                <button id="emwi-show" class="button button-large">
                    <?php echo __('从URL添加媒体文件', 'kratos'); ?>
                </button>
                <?php print_media_new_panel(true); ?>
            <?php else : ?>
                <a class="button button-large"
                   href="<?php echo esc_url(admin_url('/upload.php?page=add-external-media-without-import', __FILE__)); ?>">
                    <?php echo __('从URL添加媒体文件', 'kratos'); ?>
                </a>
            <?php endif; ?>
        </div>
...
function print_media_new_panel($is_in_upload_ui)
{
    ...
    <div id="emwi-media-new-panel" <?php if ($is_in_upload_ui) : ?>style="display: none"<?php endif; ?>>
        <label id="emwi-urls-label"><?php echo __('从URL添加媒体文件', 'kratos'); ?></label>
        <textarea id="emwi-urls" rows="<?php echo $is_in_upload_ui ? 3 : 10 ?>" name="urls" required
                  placeholder="<?php echo __("填写要添加媒体的URL地址. \n 一行一个.", 'kratos'); ?>"
                  value="<?php if (isset($_GET['urls'])) echo esc_url($_GET['urls']); ?>"></textarea>
        <div id="emwi-hidden"
            ...
            <div id="emwi-properties">
                ...
                <input id="emwi-width" name="width" type="number"
                       value="<?php if (isset($_GET['width'])) echo esc_html($_GET['width']); ?>">
                <label><?php echo __('高', 'kratos'); ?></label>
                <input id="emwi-height" name="height" type="number"
                       value="<?php if (isset($_GET['height'])) echo esc_html($_GET['height']); ?>">
                <label><?php echo __('文件类型', 'kratos'); ?></label>
                <input id="emwi-mime-type" name="mime-type" type="text"
                       value="<?php if (isset($_GET['mime-type'])) echo esc_html($_GET['mime-type']); ?>">
            </div>
        </div>
        <div id="emwi-buttons-row">
            <input type="hidden" name="action" value="add_external_media_without_import">
            <span class="spinner"></span>
            <input type="button" id="emwi-clear" class="button" value="<?php echo __('清除', 'kratos') ?>">
            <input type="submit" id="emwi-add" class="button button-primary" value="<?php echo __('添加', 'kratos') ?>">
            <?php if ($is_in_upload_ui) : ?>
                <input type="button" id="emwi-cancel" class="button" value="<?php echo __('取消', 'kratos') ?>">
            <?php endif; ?>
        </div>
    </div>
    <?php
